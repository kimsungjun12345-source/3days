...
function askReturn(goal, from) {
  ...
  /* '다시 3일'이 아니라 '다음 3일'이다 — 안내 넷째 장에 적어 둔 이유와 같다.
     되갚는 것이 아니라 이어지는 차례라는 게 하필 이 화면에서 가장 중요하다. */
...
const ONBOARD = [
  ...
  },
  {
    ...
    /* '다시 3일'이라고 쓰지 않는다.
     *
     * 스트릭 앱을 써 본 사람에게 '다시'는 '처음부터 다시', 곧 벌칙이다.
     * 안심시키려고 놓은 문장이 정확히 그 두려움을 건드리고 있었다.
     * '다음 3일'이면 잃은 것을 되갚는 게 아니라 그냥 이어지는 다음 차례다.
     *
     * '이어서'는 그대로 두되 '그대로 이어져요'까지 가지는 않는다 —
     * restart()는 checks를 비우므로 채우던 칸은 이어지지 않는다. 두 칸 채우고
     * 끊긴 사람이 두 칸부터 이어질 거라 읽으면 다음 화면에서 어긋난다.
     * 이어지는 것은 탑이고, 3일은 새로 센다. 옆 그림이 하는 말 그대로다. */
    body: "며칠 빠뜨려도 쌓아 둔 돌은 없어지지 않아요.\n<b>돌아오면 이어서 다음 3일이에요.</b>",
  },
  {
