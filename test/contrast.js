...
(async () => {
  ...
  for (const combo of COMBOS) {
    ...
    const screens = {
      ...
      // 3일을 채운 순간 먼저 뜨는 화면
      "돌 고르기": async () => { await page.evaluate(() => { closeDetail(); openStonePick(state.goals[0]); }); },
      // 눌러 볼 것은 하나뿐이고, 안 하겠다는 뜻은 모서리의 ✕로 말한다
      "축하": async () => { await page.evaluate(() => { closeStonePick(); heldGoalId = null; render(); showCheer(state.goals[0]); }); },
      // 탑을 세운 날에만 뜨는 백업 권유
